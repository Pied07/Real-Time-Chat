...
export default function ChatPage() {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const msgs: Message[] = snapshot.docs.map((doc) => {
        ...
        return {
          ...
          time: new Date(data.createdAt?.toDate() || Date.now()).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(typingRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        const lastTyped = data.lastTyped?.toDate?.() || new Date();
        setIsTyping(data.isTyping && Date.now() - lastTyped.getTime() < 4000);
      } else {
        setIsTyping(false);
      }
    });
  ...
  const handleTyping = () => {
    ...
    setDoc(typingRef, { isTyping: true, lastTyped: serverTimestamp() });

    setTimeout(() => {
      setDoc(typingRef, { isTyping: false, lastTyped: serverTimestamp() });
    }, 3000);
  };
          ...
          <>
            <div className="h-20 border-b border-white/10 px-8 flex items-center justify-between bg-zinc-950 flex-shrink-0">
              <div className="flex items-center gap-4">
                <div className="relative">
                  {selectedUser.avatar ? (
                    <img src={selectedUser.avatar} alt="" className="w-11 h-11 rounded-2xl object-cover" />
                  ) : (
              ...
              <div className="flex items-center gap-3">
                <button onClick={() => alert(`Voice call with ${selectedUser.name}`)} className="flex items-center gap-3 px-6 py-3 bg-zinc-900 hover:bg-zinc-800 border border-white/10 rounded-2xl transition-all">
                  <Phone className="w-5 h-5 text-emerald-400" />
                  <span className="font-medium">Voice</span>
                </button>
                <button onClick={() => alert(`Video call with ${selectedUser.name}`)} className="flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-violet-600 to-fuchsia-600 rounded-2xl font-semibold hover:scale-105 transition-all">
                  <Video className="w-5 h-5" />
                  <span>Video Call</span>
                </button>
                <button onClick={() => setShowProfile(true)} className="flex items-center gap-3 px-6 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl transition-all">
            ...
            {/* Messages - Fixed Emoji Display */}
            <div className="flex-1 p-8 overflow-y-auto space-y-8 bg-zinc-950 scrollbar-hide min-h-0">
                ...
                messages.map((msg) => (
                  <div key={msg.id} className={`flex ${msg.isMe ? "justify-end" : "justify-start"}`}>
                    ...
                    <div className={`max-w-[65%]`}>
                      <div className={`text-xs text-gray-500 mb-1.5 px-2 ${msg.isMe ? "text-right" : "text-left"}`}>
                        {msg.isMe ? "You" : msg.senderName} • {msg.time}
                      </div>
                      <div
                        className={`px-6 py-4 rounded-3xl text-[17px] leading-relaxed whitespace-pre-wrap break-words font-['system-ui'] ${
                          msg.isMe
            ...
            <div className="p-6 border-t border-white/10 bg-zinc-950 flex-shrink-0">
              <div className="bg-zinc-900 rounded-3xl px-6 py-4 flex items-start gap-4 relative">
                <button className="text-gray-400 hover:text-white p-2 mt-1">
                ...
                <textarea
                  ...
                  className="flex-1 bg-transparent outline-none text-lg placeholder:text-gray-500 resize-y min-h-[24px] max-h-[160px] py-3 scrollbar-hide"
                  rows={1}
                  disabled={sending}
                />
                ...
                <button
                  ...
                  className="bg-white text-black px-8 py-3 rounded-2xl font-semibold flex items-center gap-2 hover:bg-white/90 disabled:opacity-50 self-end"
                >
                  {sending ? "Sending..." : "Send"} <Send className="w-5 h-5" />
                </button>

                {showEmojiPicker && (
                  <div ref={pickerRef} className="absolute bottom-full left-12 mb-2 z-50 shadow-2xl">
                    <EmojiPicker
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
          <div className="bg-zinc-900 border border-white/10 rounded-3xl w-full max-w-md p-8">
            <div className="flex justify-end">
              <button onClick={() => setShowProfile(false)} className="text-3xl text-gray-400 hover:text-white">×</button>
            </div>
            <div className="flex flex-col items-center text-center">
              ...
              <button onClick={() => setShowProfile(false)} className="w-full py-4 bg-white/10 hover:bg-white/20 rounded-2xl font-semibold">
                Close
              </button>
            </div>
          </div>
